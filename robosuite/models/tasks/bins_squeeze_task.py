...
class BinSqueezeTask(Task):
    """
    Creates MJCF model of a bin packing task.
    A bin packing task consists of one robot picking objects from the table
    and placing them into a bin. This class combines the robot, the
    arena, and the objects into a single MJCF model of the task.
    """

    def __init__(self, mujoco_arena, mujoco_robot, mujoco_objects, visual_objects, obj_poses):
        """
        Args:
    ...
    def move_objects_oracle(self):
        """Places objects randomly until no collisions or max iterations hit."""
        placed_objects = []
        index = 0
        count = 0
        prev_horizontal_radius = 0.0
        ...
        for _, obj_mjcf in self.mujoco_objects.items():
            ...
            for _ in range(1):  # 5000 retries
                object_x = -bin_x_half + x_offset + horizontal_radius + 0.01
                object_y = -bin_y_half + prev_horizontal_radius + horizontal_radius + 0.01

                if object_x > bin_x_half:
                    prev_horizontal_radius = 0.0
                    x_offset += 2 * horizontal_radius
                    object_x = -bin_x_half + x_offset + horizontal_radius + 0.01
                    object_y = -bin_y_half + prev_horizontal_radius + horizontal_radius + 0.01

                prev_horizontal_radius += 2 * (horizontal_radius + 0.002)
